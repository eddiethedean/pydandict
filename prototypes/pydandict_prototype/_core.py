"""Integrated feasibility experiment, not a production package.
...
_BUILDING: ContextVar[bool] = ContextVar("pydandict_prototype_building", default=False)
_CANONICAL: ContextVar[bool] = ContextVar(
    "pydandict_prototype_canonical", default=False
)
_MISSING = object()
_IMMUTABLE = {
    type(None),
...
def _data(model: Any) -> dict[str, Any]:
    ...
    data = {
        k: v
        for k, v in object.__getattribute__(model, "__dict__").items()
        if k in fields
    }
...
def _clone(
    ...
    if identity in active:
        raise TypeError("cyclic values are outside the prototype ownership envelope")
    active.add(identity)
    try:
        if isinstance(value, DictModel):
            ...
            result = {
                _clone(k, None, active): _clone(v, origins, active)
        ...
        if origins is not None and (
            isinstance(value, Owned) or _root(value) is not None
        ):
...
def _prepare(
    ...
    def prepare(before: Any, after: Any, path: tuple[Any, ...]) -> Any:
        ...
        if check_untouched and _fingerprint(before) != _fingerprint(after):
            raise TypeError(
                f"canonical revalidation changed untouched state at {path!r}; use idempotent validators"
            )
        if isinstance(after, DictModel):
            ...
            updates.extend(
                [
                    ...
                    (
                        target,
                        "_pd_version",
                        getattr(target, "_pd_version", 0)
                        + (tuple(values) != previous_keys),
                    ),
                ]
            )
            ...
            if isinstance(after, list):
                ...
                values = [
                    prepare(
                        previous[i] if i < len(previous) else _MISSING, v, path + (i,)
                    )
    ...
    for node in getattr(root, "_pd_nodes", ()):
        if id(node) not in retained:
            updates.append(
                (node, "_pd_alive" if isinstance(node, DictModel) else "_alive", False)
            )
...
def _commit(updates: list[tuple[Any, str, Any]]) -> None:
    ...
    undo = [
        (target, name, getattr(target, name, _MISSING)) for target, name, _ in updates
    ]
    try:
...
def _install(value: Any) -> Any:
    if value.__pydantic_extra__ and type(value).model_config.get("extra") != "allow":
        raise TypeError(
            "construction extra override conflicts with ongoing class policy"
        )
...
class DictModel(BaseModel, MutableMapping[str, object]):
        ...
        def finish(value: Any, next_validator: Any) -> Any:
            if _BUILDING.get():
                if isinstance(value, DictModel):
                    ...
                    object.__setattr__(
                        result, "__pydantic_fields_set__", value.model_fields_set
                    )
                    return result
    ...
    def __pydantic_init_subclass__(cls, **kwargs: Any) -> None:
        ...
        if cls.model_config.get("revalidate_instances") != "always":
            raise TypeError("prototype requires revalidate_instances=always")
        ...
        if cls.__private_attributes__:
            raise TypeError("private attributes are not supported by this prototype")
        if cls.model_post_init is not BaseModel.model_post_init:
            raise TypeError("model_post_init is outside the prototype hook contract")
        ...
        if any(
            ...
            raise TypeError(
                "writable properties are outside the prototype transaction API"
            )
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        if _root(self) is None:
            ...
            raise TypeError("private writes are outside the prototype API")
        else:
            self[name] = value
    ...
    def _check_write(
        self, key: str, *, remove: bool = False, reset: bool = False
    ) -> None:
        if not isinstance(key, str):
            raise TypeError("model keys must be strings")
        ...
        for key in path:
            if isinstance(value, DictModel):
                if value.model_config.get("frozen") or (
                    key in type(value).model_fields
                    and type(value).model_fields[key].frozen
                ):
                    _policy(
                        value, "frozen_instance", key, "Frozen ancestor cannot change"
                    )
            ...
            for change in absolute:
                ...
                for key in change:
                    if isinstance(node, DictModel):
                        object.__getattribute__(node, "__pydantic_fields_set__").add(
                            key
                        )
    ...
    def update(
        ...
        def apply(draft: Any):
            ...
            object.__setattr__(
                draft, "__pydantic_extra__", replacement.__pydantic_extra__
            )
    ...
    def copy(
        ...
        *,
        ...
        warnings.warn(
            "copy is deprecated; use model_copy", DeprecationWarning, stacklevel=2
        )
    ...
    def __reduce_ex__(self, protocol: SupportsIndex) -> Any:
        raise TypeError("pickle is outside the prototype ownership contract")
