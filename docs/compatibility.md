...
compatibility on the pinned experimental stack. The matrix below records the
Phase 0.2 production contract. V1 targets Pydantic v2 only, pinned to the version
declared in package metadata; no other version is silently accepted. The planning
probes use Python
...
Phase 0.2 provides a fail-closed compatibility boundary in
...
The adapter owns blank snapshot allocation, raw storage reads/swaps/restoration,
canonical and alias schema rewrites, serializer delegation and compiled caches.
Consumed fields, storage, fields-set, model schema entries and validators are
checked before use. Caches belong to each model class, track schema identity and
are invalidated by a successful `model_rebuild`, including before its first instance.
...
| Private attributes | Declared private attributes are rejected | Custom initialization, finalizers, model_post_init and writable properties are also unsupported |
...
| Trusted construction | `model_construct` is rejected in Phase 0.2 | Intentional narrower API |
...
serializers. Test fields that are excluded, renamed, computed,
...
PydanDict preserves those boundaries within its supported value envelope. A custom model serializer
...
[alias guide](https://docs.pydantic.dev/latest/concepts/alias/). The Phase 0.2
...
probe demonstrates their unchecked behavior. PydanDict copy validates
...
methods. The deprecated `copy()` with only supported update/deep arguments
...
The supported Python target starts at 3.11, as declared in package metadata.
